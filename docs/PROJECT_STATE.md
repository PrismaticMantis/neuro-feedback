...
- Active branch: `eeg-multidevice-refactor`.
...
- **Docs restructured.** Split into `README.md` (mechanics), `docs/PROJECT.md`
  (vision/philosophy), and this file (living state). Rebranded NeuroFlo →
  NeuroSymphony across docs/comments (live domain left intact).
...
3. Decide whether to merge `eeg-multidevice-refactor` → `main`.

## Session Handoff

> "Where the screwdriver was left." The practical state for the next person.

- **Current branch:** `eeg-multidevice-refactor`
- **Safe stopping point:** BrainBit channel diagnostics complete; Capacitor iOS
  audio loading fixed; documentation ecosystem in place. No known merge conflicts.
- **Next developer should:**
  - `git fetch && git checkout eeg-multidevice-refactor && git pull`
